 ...
 *
 * @param  {String} projectLink [optional] Project URL
 * @return {Promise}
 */
Project.prototype.load = function(projectLink) {
    projectLink = projectLink || this.data.links.self;

    return this.api.request(projectLink).then(function(res) {
 ...
 *
 * @param  {String} projectLink [optional] Project URI
 * @return {Promise}
 */
Project.prototype.delete = function(projectLink) {
    projectLink = projectLink || this.data.links.self;

    return this.api.request(projectLink, 'DELETE').then(function() {
        this.data = null;
    }.bind(this));
};
            ...
            if (roleUri) {
                return api.request(roleUri).then(function(res) {
                    if (res.data.projectRole.meta.title === name) {
                        res.data.projectRole.links.self = roleUri;
