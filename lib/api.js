...
API.prototype.request = function(url, method, data) {
    ...
    if (this.__debug) {
        console.log('http %s %s', options.method, options.path);
    }
    
    return request(options, data).then(function(res) {
        if (this.__debug) {
            console.log('http %s %s %d', options.method, options.path, res.statusCode);
        }
        
        if (res.statusCode === 401 && !url.match('^/gdc/account/token') && !url.match('^/gdc/account/login')) { // TT token validity has expired
            if (this.__debug) {
                console.log('requesting fresh temporary token');
            }
